...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        option
            .setName("date")
            .setDescription("Must be later than 1995-06-16, and earlier than tomorrow. YYYY-MM-DD.")
            .setRequired(false)
        )
    ...
    async execute(interaction) {
        ...
        const desc = interaction.options.getBoolean("descriptor") == null ? true : interaction.options.getBoolean("descriptor");
        ...
        await axios ({
            ...
            .then((res) => {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields(
                        { name: "Author", value: `${(res.data.copyright) ? res.data.copyright : "unknown"}`, inline: true },
                    ...
                    .setFooter({ text: ((`${date}`.length == 10) ? "‎ " : "If you optioned for a custom date, make sure you enter it per format.") });
            ...
            .catch((err) => {
                const embed = new EmbedBuilder()
                    .setTitle(`Error`)
                    .addFields(
                        { name: "Details", value: "The website did not respond. Please check your parameters!" },
                        { name: "Additional info", 
                        value: typeof(err) == JSON
