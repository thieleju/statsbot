...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addStringOption(option =>
			option.setName('anime').setDescription('The name of an anime you\'d like a quote from'),
		),
	async execute(interaction) {
		const animeName = interaction.options.getString('anime');
		await axios({
			method: 'get',
			url: animeName ? `https://animechan.vercel.app/api/quotes/anime?title=${animeName}` : 'https://animechan.vercel.app/api/random',
			responseType: 'json',
		})
			.then((response) => {
